...
- **M3-1 高频容器（M3 收尾扩展）** —— 内容生成 ✅（2026-08-14）；**A 区热身练习 1-4 review 通过（2026-08-17）**：编译 0 警告 + 输出全对；02/03 待跑 format、04 待补注释答案；**待写 B 区 5-9**。
- **M6 · gdb 调试入门** —— 文档/示例/练习已生成，用户已在 VS Code 可视化调试走通断点/单步；`01_debug_me.cpp` 越界 bug 待独立定位（可随时并行练）。

**下一步 Next：**

1. **写 M3-1 B 区练习 5-9**（05 去重+存在性 / 06 括号匹配 / 07 约瑟夫环 / 08 前 K 大 / 09 deque 回文），写完回来 review → C 区 10 综合 → 自评表全 ✅ 收官。
