...
- 路径：`docs/NN[-N]-短横线主题.md`，与 `examples/NN[-N]/`、`practice/NN[-N]/` 编号一致。**命名规则（2026-08-10 定，2026-08-11 双端合并后校准、同日后重新启用 NN-N）**：目录/文档 = 交付块号——整块交付的模块用 `NN`（M1→`01`、M2→`02`、M3→`03`、M6→`06`），拆分模块用 `NN-N`（M3→`03-1`、M4→`04-1`/`04-2`、M5→`05-1`/`05-2`、M7→`07-1`/`07-2`）；不建空壳目录。
...
- **练习设计原则（2026-08-11 用户定）**：练习**数量不设上限**，按「由浅入深」分层设计——先基础热身（每个语法点最小可用题，跑通即可），再核心应用（每个容器的典型用途），最后综合/学后练习（跨知识收口 + 贴近刷题的选型题）。目标是对该节知识**全面覆盖**，不要「每容器各一题」地凑数；练习文件仍按 `NN_主题.cpp` 顺序编号。
- **优先级：代码是重点，文档是记录**（用户 2026-08-11 确认）：文档的价值在**内容准确、与代码/现状一致**（路径、模块号、说法不跑偏），不在排版完美。markdownlint 是辅助**不是门槛**——`.markdownlint.jsonc` 已关 MD013（行长，对中文项目无意义，2026-08-10 确认）；结构规则（表格分隔行 padded `| --- |`、代码块带语言标签、列表/代码块前后空行）写文档时照着来即可，个别小告警不纠结。**格式化规范只约束代码**（`.clang-format` + `tools/format.*`）。
