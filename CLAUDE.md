...
- 路径：`docs/NN[-N]-短横线主题.md`，与 `examples/NN[-N]/`、`practice/NN[-N]/` 编号一致。**命名规则（2026-08-10 定，2026-08-11 双端合并后校准）**：目录/文档 = 交付块号——整块交付的模块用 `NN`（当前全部模块皆如此：M1→`01`、M2→`02`、M3→`03`、M6→`06`），拆分模块用 `NN-N`（目前未启用，需要拆分交付时再启用）；不建空壳目录。
